...
func (r *OAuthProxyReconciler) handleDaemonSetChanges(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	...
	if err != nil {
		// The daemonset may not have been created yet, so requeue
	...
	if err != nil {
		delay := 5 * time.Second
		return &ctrl.Result{RequeueAfter: delay}, errors.Wrap(err, "Unable to CreateOrUpdate DaemonSet")
	}
...
func (r *OAuthProxyReconciler) finalizeDaemonSet(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	found := &appsv1.DaemonSet{}
	err := r.Client.Get(context.TODO(), types.NamespacedName{
		Name:      cr.Spec.ResourceName,
		Namespace: cr.Spec.Namespace,
