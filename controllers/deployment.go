...
func (r *OAuthProxyReconciler) handleDeploymentChanges(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	...
	if err != nil {
		delay := 5 * time.Second
		return &ctrl.Result{RequeueAfter: delay}, errors.Wrap(err, "Unable to CreateOrUpdate Deployment")
	}
...
func (r *OAuthProxyReconciler) finalizeDeployment(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	found := &appsv1.Deployment{}
	err := r.Client.Get(context.TODO(), types.NamespacedName{
		Name:      cr.Spec.ResourceName,
		Namespace: cr.Spec.Namespace,
