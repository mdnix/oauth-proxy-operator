...
func (r *OAuthProxyReconciler) handleDeploymentConfigChanges(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	...
	if err != nil {
		// The deploymentconfig may not have been created yet, so requeue
	...
	if err != nil {
		delay := 5 * time.Second
		return &ctrl.Result{RequeueAfter: delay}, errors.Wrap(err, "Unable to CreateOrUpdate DeploymentConfig")
	}
...
func (r *OAuthProxyReconciler) finalizeDeploymentConfig(cr *oauthv1alpha1.OAuthProxy) (*ctrl.Result, error) {
	found := &oappsv1.DeploymentConfig{}
	err := r.Client.Get(context.TODO(), types.NamespacedName{
		Name:      cr.Spec.ResourceName,
		Namespace: cr.Spec.Namespace,
